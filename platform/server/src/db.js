...
const HELP_SEEDS = [
  ...
  ['worker', '接单', '交付时要填什么、传什么（按工种）', '交付页会按你承接的工种自动列出"要填写的字段"和"要上传的材料"，不同工种不一样：如设计要传成果预览图、配送要传签收照片、家政要传服务前后照片、数据标注要填标注数量并传成果文件。带星号(*)的为必填/必传，缺了无法提交、企业也无法验收。按提示如实填写上传，既是交付凭证，也在争议时保护你。'],
  ['company', '合规', '按工种结构化验收，交付物怎么看', '平台对 22 个任务类目配置了专属交付清单，零工按工种结构化提交。任务详情的"交付物"区会按字段逐项展示其填写内容，并把上传材料分组列出、可逐个下载核对；必填必传项零工不提交则无法交付到验收环节。如需调整某工种的交付要求，可在运营端「交付物模板」在线编辑（即时生效、自动留痕）。']
]
