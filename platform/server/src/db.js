...
// 法律文书与合同模板（占位符 {{name}} 在签署时渲染为实际值并存档快照）
const LEGAL_SEEDS = [
  ['tos', '平台服务协议', `第一条 本平台采用"承揽后分包"模式：用工企业与平台签订《总承揽框架合同》，平台与零工签订《分包协议》。平台为交易主体与责任主体。
第二条 用户注册即表示已阅读并同意本协议全部条款，平台依法对用户身份信息进行核验并妥善保管。
第三条 零工通过平台承接任务属于经营性承揽行为，与用工企业、平台均不构成劳动关系。
第四条 平台依据税收法律法规对自然人零工的劳务报酬代扣代缴个人所得税，并按规定向税务机关报送涉税信息。
第五条 资金由持牌银行存管，平台不触碰在途资金；结算以任务验收为前提。
第六条 用户应妥善保管账号，因账号外借产生的损失由用户自行承担。
第七条 本协议未尽事宜按相关法律法规执行。`],
  ['privacy', '隐私政策', `一、我们收集的信息：注册手机号、实名认证所需的身份证信息与银行卡号（仅传输至持牌核验机构，平台存储脱敏掩码）、交易与结算记录。
二、信息用途：身份核验、合同签署、结算支付、依法纳税申报与涉税信息报送。
三、信息共享：仅向公安实名核验机构、存管银行、税务机关、电子签与保险服务机构等依法依约共享必要信息。
四、存储期限：依据税收征管要求，交易与凭证类数据保存不少于10年。
五、您的权利：可查询、更正个人资料；注销账号请联系平台客服，依法可删除的信息将被删除。`],
  ['master', '总承揽框架合同', `甲方（用工企业）：{{partyA}}（统一社会信用代码：{{licenseNo}}）
乙方（平台）：灵工云平台
合同编号：{{contractNo}}    签署日期：{{date}}

第一条 合作模式：甲方将其零散业务需求整体发包予乙方承揽，乙方以自身名义组织实名零工完成，并对交付成果向甲方负责。
第二条 工单机制：甲方每笔任务通过平台生成《任务工单》，作为本框架合同附件，与本合同具有同等效力。
第三条 费用与发票：甲方按工单约定承揽价向银行存管账户充值；任务验收合格后由乙方全额开具增值税发票（现代服务），甲方凭票税前扣除。
第四条 资金存管：全部资金经持牌银行存管虚拟账户划转，乙方仅发起分账指令。
第五条 合规承诺：甲方保证业务真实，不得借平台虚构交易；甲方不得对零工进行考勤管理或约定固定薪资。
第六条 责任划分：零工履约过程中的人身意外由乙方投保的按单保险保障；甲方对交付成果享有验收权，成果不合格不计酬。
第七条 本合同经电子签名成立，存证哈希：{{hash}}。`],
  ['frame_sub', '分包协议（框架）', `甲方（平台）：灵工云平台
乙方（零工）：{{partyB}}
协议编号：{{contractNo}}    签署日期：{{date}}

第一条 合作性质：乙方以独立承揽人身份承接甲方分包的任务，双方为承揽合同关系，不构成劳动、劳务派遣或雇佣关系。
第二条 计酬方式：按成果、按件或按单计酬，不约定固定工作时间、不设底薪。
第三条 工作方式：乙方自主安排工作时间与地点，工具设备自备，不接受任何一方的考勤管理。
第四条 成果验收：交付成果须符合工单约定标准，成果不合格不计酬；乙方可在驳回后修改重新交付。
第五条 税费：乙方为自然人的，甲方依法代扣代缴个人所得税并代办增值税事项；乙方为个体工商户的，自行申报经营所得并向甲方开具发票。
第六条 保障：乙方每次接单由甲方投保意外保险（接单生效、交付终止）。
第七条 每次接单生成《分包工单》作为本协议附件。本协议经电子签名成立，存证哈希：{{hash}}。`],
  ['work_order', '任务工单', `任务工单（《总承揽框架合同》附件）
工单编号：{{contractNo}}    生成日期：{{date}}
发包企业：{{partyA}}
任务名称：{{taskTitle}}
任务类目：{{category}}    计酬方式：{{payMethod}}
承揽价：人民币 {{price}} 元（已冻结于银行存管账户）
交付截止：{{deadline}}
交付标准：{{standard}}
本工单为《总承揽框架合同》之附件，与框架合同具有同等法律效力。存证哈希：{{hash}}。`],
  ['sub_order', '分包工单', `分包工单（《分包协议（框架）》附件）
工单编号：{{contractNo}}    生成日期：{{date}}
承接零工：{{partyB}}
任务名称：{{taskTitle}}
计酬方式：{{payMethod}}（按成果计酬，成果不合格不计酬）
分包报酬：人民币 {{subPrice}} 元（验收合格后由存管银行结算，自然人依法代扣税费）
保险：本单已投保意外险（保单号 {{policyNo}}），接单生效、交付终止。
本工单为《分包协议（框架）》之附件，经电子签名成立。存证哈希：{{hash}}。`]
]
