<template>
  <div>
    <PageHeader
      title="发票中心"
      subtitle="平台开具的全部数电专用发票均归档于此，可查看明细与打印"
    />

    <el-alert type="success" show-icon :closable="false" class="tip-alert">
      <template #title>验收即自动全额开具数电发票，凭票即可企业所得税税前扣除</template>
      <div class="tip-desc">
        任务验收通过后，平台按任务全额向贵司开具 6% 现代服务<TermTip
          term="数电票"
          text="数电专用发票"
        />，与框架合同、工单、结算确认单共同构成<TermTip term="四流" text="四流合一" />证据链。
      </div>
    </el-alert>
      ...
      <el-table v-loading="loading" :data="list" stripe>
        <el-table-column prop="no" label="发票号" width="200">
          <template #default="{ row }"
            ><span class="mono">{{ row.no }}</span></template
          >
        </el-table-column>
        ...
        <el-table-column prop="item" label="开票项目" min-width="140" show-overflow-tooltip />
        <el-table-column label="开票日期" width="170" show-overflow-tooltip>
        ...
        <template #empty>
          <el-empty description="暂无发票">
            <div class="empty-hint">
              任务验收通过后，平台将自动按任务全额开具 6% 数电专用发票并归档于此。
            </div>
          </el-empty>
        </template>
      </el-table>
    ...
    <el-dialog v-model="detailVisible" title="发票详情" width="600px">
      ...
      >
        ...
        <div class="void-detail">
          <span
            >红字发票号：<span class="mono">{{ current.redInvoiceNo || '—' }}</span></span
          >
          <span v-if="current.voidReason" class="void-reason"
            >红冲原因：{{ current.voidReason }}</span
          >
        </div>
      </el-alert>
      <el-descriptions v-if="current" :column="1" border>
        ...
        <el-descriptions-item label="购方抬头">{{
          current.buyer?.title || '—'
        }}</el-descriptions-item>
        ...
        <el-descriptions-item label="关联结算确认单">{{
          current.confirmNo || '—'
        }}</el-descriptions-item>
        <el-descriptions-item label="开票日期">{{
          fmtDateTime(current.issuedAt)
        }}</el-descriptions-item>
      </el-descriptions>
      <template #footer>
        ...
        <el-tooltip
          content="该发票已红冲作废"
          :disabled="current?.status !== 'voided'"
          placement="top"
        >
          <span class="print-wrap">
            <el-button
