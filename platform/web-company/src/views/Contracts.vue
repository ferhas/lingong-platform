    ...
    <el-table :data="list" v-loading="loading" stripe>
      ...
      <el-table-column label="合同类型" width="170">
        <template #default="{ row }">
          <el-tag :type="typeTag(row.type)" effect="light">{{ CONTRACT_TYPE[row.type] || row.type }}</el-tag>
        </template>
      </el-table-column>
        ...
        <template v-if="current">
          ...
          <el-descriptions :column="2" border size="small" class="meta-desc">
            <el-descriptions-item label="合同类型">
              <el-tag :type="typeTag(current.type)" effect="light" size="small">{{ CONTRACT_TYPE[current.type] || current.type }}</el-tag>
            ...
            <el-descriptions-item label="签署时间">{{ fmtDateTime(current.signedAt) }}</el-descriptions-item>
