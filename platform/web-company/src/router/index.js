...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'publish', name: 'publish', component: () => import('../views/Publish.vue'), meta: { title: '发布任务', noFinance: true } },
      { path: 'batch-publish', name: 'batchPublish', component: () => import('../views/BatchPublish.vue'), meta: { title: '批量发单', noFinance: true } },
      ...
      { path: 'members', name: 'members', component: () => import('../views/Members.vue'), meta: { title: '成员管理', requiresOwner: true } },
...
router.beforeEach(to => {
  ...
  // 角色路由门禁：成员管理仅企业主；发布/批量发单财务无权（与后端 requireCompanyRole 一致）
  if (token) {
    const memberRole = JSON.parse(localStorage.getItem('user') || 'null')?.memberRole
    if (to.meta?.requiresOwner && memberRole && memberRole !== 'owner') return { path: '/dashboard' }
    if (to.meta?.noFinance && memberRole === 'finance') return { path: '/dashboard' }
  }
